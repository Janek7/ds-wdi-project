...
public class GameModeFuser extends AttributeValueFuser<List<GameMode>, VideoGame, Attribute> {

	public GameModeFuser(ConflictResolutionFunction resolutionFunction) {
	...
	public void fuse(RecordGroup<VideoGame, Attribute> group, VideoGame fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
		...
		// get the fused value
		FusedValue<List<GameMode>, VideoGame, Attribute> fused = getFusedValue(group, schemaCorrespondences, schemaElement);

		// set the value for the fused record
		fusedRecord.setGameModes(fused.getValue());

		// add provenance info
		fusedRecord.setAttributeProvenance(VideoGame.GAME_MODES, fused.getOriginalIds());
	}

	@Override
	public boolean hasValue(VideoGame record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(VideoGame.GAME_MODES);
	}

	@Override
	public List<GameMode> getValue(VideoGame record, Correspondence<Attribute, Matchable> correspondence) {
		return record.getGameModes();
	}
