...
public class GenreFuser extends AttributeValueFuser<List<Genre>, VideoGame, Attribute> {

	public GenreFuser(ConflictResolutionFunction resolutionFunction) {
	...
	public void fuse(RecordGroup<VideoGame, Attribute> group, VideoGame fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
		...
		// get the fused value
		FusedValue<List<Genre>, VideoGame, Attribute> fused = getFusedValue(group, schemaCorrespondences, schemaElement);

		// set the value for the fused record
		fusedRecord.setGenres(fused.getValue());

		// add provenance info
		fusedRecord.setAttributeProvenance(VideoGame.GENRES, fused.getOriginalIds());
	}

	@Override
	public boolean hasValue(VideoGame record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(VideoGame.GENRES);
	}

	@Override
	public List<Genre> getValue(VideoGame record, Correspondence<Attribute, Matchable> correspondence) {
