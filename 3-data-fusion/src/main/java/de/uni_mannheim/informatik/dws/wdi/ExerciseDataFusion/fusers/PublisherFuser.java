...
public class PublisherFuser extends AttributeValueFuser<String, VideoGame, Attribute> {

	public PublisherFuser(ConflictResolutionFunction resolutionFunction) {
	...
	public void fuse(RecordGroup<VideoGame, Attribute> group, VideoGame fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
		...
		// add provenance info
		fusedRecord.setAttributeProvenance(VideoGame.PUBLISHER, fused.getOriginalIds());
	}

	@Override
	public boolean hasValue(VideoGame record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(VideoGame.PUBLISHER);
	}
